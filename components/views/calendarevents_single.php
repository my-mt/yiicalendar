...
<div>
    ...
    <div class="modal fade" id="eventModal-<?= $year . '-' . $month ?>" tabindex="-1" role="dialog" aria-labelledby="smallModal" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-content-table">
                    <div id="table-modal-body-<?= $year . '-' . $month ?>"></div>
                </div>
...
function showEvent(arrIdrec) {
    ...
    table += '<thead class="thead-inverse"><tr><th>#</th><th>' + summary + '</th><th>Время</th>';
    try {
    ...
    arrIdrec.forEach(function(item, i) {
        ...
        if (i === 0) {
            ...
            title += date.toLocaleString("ru", optionsDateTitle) + ' / ' + rec["calendar_summary"];
        }
        ...
        if (rec["start"] === rec["end"]) {
            ...
            table += '<td>' + dateStart.toLocaleString("ru", optionsDate) + " - " + dateEnd.toLocaleString("ru", optionsDateEnd) + '</td>';
        }
    ...
    $("#eventModal-<?= $year . '-' . $month ?> #table-modal-body-<?= $year . '-' . $month ?>").html(table);


    $("#eventModal-<?= $year . '-' . $month ?> .modal-title").html(title);
//    $("#eventModal-<?= $year . '-' . $month ?> .modal-body h3").text('').text(dataEvent[0].result);
    $("#eventModal-<?= $year . '-' . $month ?>").modal('show');
}
...
//                }
            ...
            var letterTitle = 8;
            var widthTable = $('#calendar-tadev-<?= $year . '-' . $month ?>').width();
    ...
    $("#calendar-tadev-<?= $year . '-' . $month ?>").append(calendar2);
...
//     $("#calendar-tadev").after(table);
     ...
     function hideRecEvent(title) {
         $('#calendar-tadev-<?= $year . '-' . $month ?> .event-item').show();
         $('#calendar-tadev-<?= $year . '-' . $month ?> .event-item').each(function() {
     ...
     $('#calendar-tadev-<?= $year . '-' . $month ?> .event-item').hover(
     function() {
