...
echo "<small>Формат $formatVersion</small>";
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="row">
                <div class="col-xs-6">
                    <div class="form-group start-section">
                        <label class="control-label">Начало</label> <span  style="cursor: pointer; color: #23527c;" id="duplicate-date">(дублировать)</span>
                        <?php
            ...
            if (is_object($calendarFields)):
                ...
                foreach($calendarFields as $keyField => $dataField) {
                    $showDescription = false;
                    $type = 'text';
                    $textarea = 0;
                    $data = @json_decode($event->description);

                    switch ($calendarFields->$keyField->type) {
                        case 'int':
                        case 'float':
                            $type = "number";
                            break;
            ...
            ?>
                ...
                <div class="form-group description">
                    <label class="control-label description-filed"><?= $dataField->name ?></label>
                    <?php if ($textarea) { ?>
                        <textarea class="form-control description-value" rows="<?= $textarea ?>" name="<?= $keyField ?>"><?= @$data->$keyField ?></textarea>
                    <?php } else { ?>
                        <input class="form-control description-value" type="<?= $type ?>" name="<?= $keyField ?>" value="<?= @$data->$keyField ?>" step="any">
                    <?php } ?>
...
jQuery(document).ready(function(){
    ...
    function makeDescriptionStr() {
        ...
        $('div.form-group.description').each(function() {
            noData = false;
            data[$(this).children('.description-value').attr('name')] = $(this).children('.description-value').val().replace( /"/g, "'" );
        });
